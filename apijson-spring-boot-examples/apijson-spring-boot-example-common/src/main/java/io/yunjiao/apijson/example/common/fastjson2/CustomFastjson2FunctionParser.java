...
import io.yunjiao.apijson.fastjson2.Fastjson2FunctionParser;
import io.yunjiao.apijson.fastjson2.Fastjson2Parser;
...
public class CustomFastjson2FunctionParser extends Fastjson2FunctionParser {
    ...
    public int deleteCommentOfMoment(@NotNull JSONObject curObj, @NotNull String momentId) throws Exception {
        ...
        if (mid == null || mid <= 0 || curObj.getIntValue(apijson.fastjson2.JSONResponse.KEY_COUNT) <= 0) {
            return 0;
        }
        ...
        return c == null ? 0 : c.getIntValue(apijson.fastjson2.JSONResponse.KEY_COUNT);
    }
    ...
    public int deleteChildComment(@NotNull JSONObject curObj, @NotNull String toId) throws Exception {
        ...
        if (tid == null || tid <= 0 || curObj.getIntValue(apijson.fastjson2.JSONResponse.KEY_COUNT) <= 0) {
            return 0;
        }
    ...
    private JSONArray getChildCommentIdList(long tid) {
        ...
        //Comment-id[]<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        apijson.fastjson2.JSONRequest idItem = new apijson.fastjson2.JSONRequest();

        //Comment<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        apijson.fastjson2.JSONRequest comment = new apijson.fastjson2.JSONRequest();
    ...
    public Object verifyAccess(@NotNull JSONObject curObj) throws Exception {
        String role = getArgStr(apijson.fastjson2.JSONRequest.KEY_ROLE);
