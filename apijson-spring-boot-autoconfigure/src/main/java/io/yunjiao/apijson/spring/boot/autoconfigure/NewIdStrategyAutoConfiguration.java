...
import io.yunjiao.apijson.spring.boot.autoconfigure.condition.NewIdStrategyCondition;
...
public class NewIdStrategyAutoConfiguration {
    ...
    NewIdStrategy newIdCustomStrategy() {
        NewIdStrategy bean = new NewIdCustomStrategy();
        if (log.isDebugEnabled()) {
            log.debug("Configure Bean [New Id Custom Strategy]");
        }
        return bean;
    }
}
